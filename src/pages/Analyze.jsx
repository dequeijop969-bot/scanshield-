...
import { ScanSearch, Lock, Crown, Image as ImageIcon, Video as VideoIcon } from "lucide-react";import { Link } from "react-router-dom";
import ImageUploader from "@/components/analyze/ImageUploader";
import VideoUploadCard from "@/components/analyze/VideoUploadCard";
import VideoAnalysisResult from "@/components/analyze/VideoAnalysisResult";
...
export default function Analyze() {
  ...
  // URL de imagem já hospedada (ex: enviada pela extensão do Chrome) — pula
  // a etapa de upload e vai direto pra análise com essa URL.
  const [prefillFileUrl, setPrefillFileUrl] = useState(null);
  const [videoFile, setVideoFile] = useState(null);
  const [videoPreview, setVideoPreview] = useState(null);
  const [isAnalyzingVideo, setIsAnalyzingVideo] = useState(false);
  const [videoResult, setVideoResult] = useState(null);
  const [analysisType, setAnalysisType] = useState("automatico");
  ...
  // Recebe imagem já enviada pela extensão do Chrome (via ?prefill_url=...)
  // e pula direto pra pré-visualização, sem precisar re-fazer upload.
  useEffect(() => {
    const url = searchParams.get("prefill_url");
    if (url) {
      setPrefillFileUrl(url);
      setImagePreview(url);
      setMediaMode("image");
    }
  }, [searchParams]);
  ...
  const handleAnalyzeVideo = async () => {
    ...
    try {
      ...
      const result = await base44.integrations.Core.InvokeVideoLLM({
        prompt: `Você é um especialista em segurança digital e análise de fraudes.
Analise este vídeo cuidadosamente em busca de sinais de golpe, fraude, phishing, deepfake ou conteúdo enganoso.
Considere: vozes sintéticas, promessas de ganho fácil, depoimentos forjados, golpes de investimento, links suspeitos, logos falsas, urgência artificial.
...
Responda em português do Brasil de forma objetiva e detalhada.`,
        ...
        response_json_schema: {
          ...
          properties: {
            ...
            summary: { type: "string", description: "Resumo objetivo da análise em 2-3 frases" },
            red_flags: {
              type: "array",
              items: { type: "string" },
              description: "Lista de sinais de alerta encontrados no vídeo"
            },
            green_flags: {
              type: "array",
              items: { type: "string" },
              description: "Lista de pontos que indicam legitimidade (se houver)"
            },
          },
          required: ["risk_level", "summary", "red_flags"],
        },
      });
      ...
      // Redireciona pra página de detalhe do vídeo
      navigate(`${createPageUrl("VideoAnalysisDetail")}?id=${saved.id}`);
    } catch (err) {
  ...
  const handleAnalyze = async () => {
    if (!file && !prefillFileUrl) return;
    setIsAnalyzing(true);
    setAnalyzeError(null);
    setScanStep(0);
    console.log("[ScanShield] Iniciando análise. Tipo:", analysisType, prefillFileUrl ? "(via extensão)" : "");

    // Timeout de segurança: se a análise passar de 60s, mostra erro em vez de carregar pra sempre
    ...
    const timeoutId = setTimeout(() => {
      ...
      setAnalyzeError(
        "A análise está demorando mais que o esperado. Isso pode ser instabilidade no servidor de IA. Tente novamente em instantes."
      );
    }, timeoutMs);

    try {
      const [{ file_url }, allAnalyses] = await Promise.all([
        prefillFileUrl ? Promise.resolve({ file_url: prefillFileUrl }) : base44.integrations.Core.UploadFile({ file }),
...
- Você NÃO tem acesso à internet em tempo real por conta própria. Se resultados de busca real forem fornecidos abaixo (seção "RESULTADOS DE BUSCA REAL"), use-os e cite as fontes explicitamente. Se não forem fornecidos, baseie-se apenas no que você já conhece pelo treinamento e deixe isso EXPLÍCITO no laudo (ex: "com base em conhecimento prévio, sem verificação ativa em tempo real") — nunca finja ter comparado com fotos que não foram realmente fornecidas.
...
No detailed_analysis, escreva análise jornalística detalhada em markdown com pelo menos 6 parágrafos.`,
    ...
    let effectivePrompt = typePrompts[analysisType];
    let detectedCategories = [analysisType];
    let possiblePublicFigure = null;

    // ── ESTÁGIO 1: Classificador — só roda no modo "automatico" ────────────
    // Chamada leve e rápida que identifica QUAIS filtros acionar, em vez de
    // depender do usuário adivinhar o tipo certo antes de saber se é golpe.
    if (analysisType === "automatico") {
      console.log("[ScanShield] Estágio 1/2: classificando categorias...");
      try {
        const classification = await base44.integrations.Core.InvokeLLM({
          prompt: `Você é um classificador rápido de conteúdo digital brasileiro. Olhe a imagem e diga quais categorias de risco se aplicam. Não escreva laudo, apenas classifique.
...
Se houver um rosto humano na imagem E você reconhecer essa pessoa como uma figura pública/famosa (ator, cantor, político, influenciador, etc.), preencha "possible_public_figure" com o nome completo dela. Se não houver rosto, ou o rosto não for de alguém que você reconhece com confiança, deixe null — NÃO adivinhe ou chute um nome.`,
          ...
          response_json_schema: {
            ...
            properties: {
              categories: {
                type: "array",
                items: { type: "string", enum: ["golpe", "oferta", "informacao_falsa", "deepfake"] },
              },
              primary: { type: "string", enum: ["golpe", "oferta", "informacao_falsa", "deepfake", "geral"] },
              possible_public_figure: { type: ["string", "null"] },
            },
            required: ["categories", "primary"],
          },
        });

        console.log("[ScanShield] Categorias detectadas:", classification);
        const cats = (classification.categories || []).filter((c) => typePrompts[c]);
        possiblePublicFigure = classification.possible_public_figure || null;

        if (cats.length > 0) {
          // Limita a 2 filtros pra manter o prompt final enxuto e focado
          detectedCategories = cats.slice(0, 2);
          effectivePrompt = detectedCategories
            .map((cat, i) => `━━━ FILTRO ${i + 1}: ${cat.toUpperCase()} ━━━\n${typePrompts[cat]}`)
            .join("\n\n");
        } else {
          detectedCategories = ["geral"];
          effectivePrompt = typePrompts.geral;
        }
      } catch (classErr) {
        // Se o classificador falhar, cai pro modo geral em vez de travar tudo
        console.error("[ScanShield] Classificador falhou, usando análise geral:", classErr);
        detectedCategories = ["geral"];
        effectivePrompt = typePrompts.geral;
      }
    }

    const isDeepfakeDetected = detectedCategories.includes("deepfake");

    // Se o usuário escolheu "Deepfake" manualmente (sem passar pelo classificador
    // automático), ainda assim tentamos identificar o possível famoso, com uma
    // chamada leve dedicada só pra isso.
    if (isDeepfakeDetected && analysisType === "deepfake" && !possiblePublicFigure) {
      try {
        const idResult = await base44.integrations.Core.InvokeLLM({
          prompt: `Há um rosto humano nesta imagem. Se você reconhecer essa pessoa como figura pública/famosa com confiança, retorne o nome completo. Se não tiver certeza ou não houver rosto, retorne null. NÃO adivinhe ou chute.`,
          model: "gemini_3_flash",
          file_urls: [file_url],
          response_json_schema: {
            type: "object",
            properties: { possible_public_figure: { type: ["string", "null"] } },
            required: ["possible_public_figure"],
          },
        });
        possiblePublicFigure = idResult.possible_public_figure || null;
      } catch (idErr) {
        console.error("[ScanShield] Identificação de famoso falhou (não crítico):", idErr);
      }
    }

    // ── FILTRO EXTRA: busca de imagens reais quando há um famoso identificado ──
    // Roda só quando faz sentido (evita gastar cota de busca à toa).
    let faceSearchContext = "";
    if (isDeepfakeDetected && possiblePublicFigure) {
      console.log("[ScanShield] Buscando aparições verificadas de:", possiblePublicFigure);
      try {
        const searchRes = await fetch("/api/extension?action=faceSearch", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ name: possiblePublicFigure }),
        });
        const searchData = await searchRes.json();

        if (!searchData.configured) {
          faceSearchContext = `\n\n⚠️ BUSCA NA WEB NÃO CONFIGURADA: a chave de busca de imagens ainda não foi configurada no servidor. NÃO afirme ter comparado com fotos reais/verificadas — baseie-se apenas no que você já conhece sobre a aparência dessa pessoa pelo seu treinamento, deixando claro no laudo que essa comparação é feita "com base em conhecimento prévio, sem busca ativa em tempo real".`;
        } else if (searchData.results && searchData.results.length > 0) {
          const sources = searchData.results
            .slice(0, 5)
            .map((r, i) => `${i + 1}. ${r.title} — fonte: ${r.source}`)
            .join("\n");
          faceSearchContext = `\n\n🔍 RESULTADOS DE BUSCA REAL (nome pesquisado: "${possiblePublicFigure}"):\n${sources}\n\nUse essas fontes reais para basear sua comparação. No campo detailed_analysis, cite explicitamente essas fontes (nome do site) ao descrever a comparação. NÃO invente fontes além destas.`;
        } else {
          faceSearchContext = `\n\n⚠️ Busca real foi feita para "${possiblePublicFigure}", mas não retornou resultados (pode ser limitação dos sites pesquisados). NÃO afirme ter encontrado ou comparado com fotos verificadas — deixe claro no laudo que a busca não encontrou fontes e a avaliação se baseia apenas em conhecimento prévio do modelo.`;
        }
      } catch (searchErr) {
        console.error("[ScanShield] Busca de famoso falhou (não crítico):", searchErr);
        faceSearchContext = `\n\n⚠️ A busca na web falhou tecnicamente. NÃO afirme ter comparado com fotos verificadas — baseie-se apenas em conhecimento prévio, deixando isso explícito no laudo.`;
      }
    }

    console.log("[ScanShield] Estágio 2/2: análise com filtros:", detectedCategories);
    const result = await base44.integrations.Core.InvokeLLM({
      prompt: `Você é o ScanShield, um sistema de segurança digital brasileiro especializado em proteção contra golpes, fraudes, deepfakes e desinformação. Seu objetivo é analisar imagens enviadas por usuários brasileiros e emitir laudos técnicos claros, detalhados e acessíveis. Você foi treinado para identificar ameaças digitais com precisão forense e sempre responder em português do Brasil. ${effectivePrompt}${faceSearchContext}${learningContext}
...
${isDeepfakeDetected ? "- deepfake_probability: 0-100\n- deepfake_model_detected: modelo suspeito ou 'Não identificado'\n- deepfake_artifacts: array com artefatos técnicos detectados\n- NÃO preencha seller_info, product_name, price_found, estimated_market_price, price_verdict" : "- Se houver produto: preencha product_name, price_found (número), estimated_market_price (número), price_verdict\n- seller_info: informações detalhadas do vendedor se visíveis"}`,
    ...
    const saved = await base44.entities.ScreenAnalysis.create({
      image_url: file_url,
      analysis_type: isDeepfakeDetected ? "deepfake" : (result.analysis_type || detectedCategories[0] || analysisType),
    ...
    navigate(createPageUrl(`AnalysisDetail?id=${saved.id}`));
      ...
      console.error("[ScanShield] Erro ao analisar imagem:", err);
      if (!timedOut) {
        setIsAnalyzing(false);
        setAnalyzeError(
          err.message === "Failed to fetch"
            ? "Não foi possível conectar ao servidor de análise. Verifique sua internet e tente novamente."
            : err.message || "Ocorreu um erro ao analisar a imagem. Tente novamente."
        );
      }
    } finally {
      ...
      >
        ...
        <AnimatePresence mode="wait">
          <motion.h1
            ...
            className="text-3xl sm:text-4xl font-black tracking-tighter text-foreground mb-3 text-balance"
          >
            {mediaMode === "image" ? "O que você quer verificar?" : "Analisar Vídeo"}
          </motion.h1>
        </AnimatePresence>
        <AnimatePresence mode="wait">
          <motion.p
            ...
            className="text-muted-foreground max-w-md mx-auto text-pretty"
          >
            {mediaMode === "image"
              ? "Envie uma captura de tela e a IA procura golpes, fraudes e informações falsas em segundos."
              : "Envie um vídeo de até 1 minuto e a IA procura golpes, fraudes e deepfakes."}
          </motion.p>
        </AnimatePresence>
        {!limitsLoading && mediaMode === "image" && (
          <motion.div
            ...
            className="mt-4 inline-flex items-center gap-2.5 text-xs text-muted-foreground bg-card border border-border px-4 py-2 rounded-full"
          >
            <ScanSearch className="w-3.5 h-3.5" />
            <span className="font-mono font-bold text-foreground">{monthlyAnalyses}/{limits.analyses}</span>
            análises usadas este mês
            <span className="w-20 h-1.5 rounded-full bg-foreground/10 overflow-hidden">
              <motion.span
                initial={{ width: 0 }}
                animate={{ width: `${Math.min(100, (monthlyAnalyses / limits.analyses) * 100)}%` }}
                transition={{ duration: 0.8, delay: 0.3 }}
                className={`block h-full rounded-full ${monthlyAnalyses / limits.analyses >= 0.8 ? "bg-red-400" : "bg-emerald-400"}`}
              />
            </span>
          </motion.div>
        )}
      ...
      {!limitsLoading && !canAnalyze ? (
        ...
        <div className="space-y-6">
          ...
          {analyzeError && !isAnalyzing && (
            <div className="rounded-xl border border-red-500/30 bg-red-500/10 p-4 flex flex-col sm:flex-row sm:items-center gap-3">
              <p className="text-sm text-red-400 flex-1">{analyzeError}</p>
              <Button
                onClick={handleAnalyze}
                variant="outline"
                size="sm"
