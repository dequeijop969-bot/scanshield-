  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8 sm:py-12">
      ...
      <div className="flex items-center gap-4 my-8">
        ...
        <span className="text-xs text-muted-foreground font-medium">ou analise um vídeo</span>
        <div className="flex-1 border-t border-border" />
      </div>
      ...
      <div className="space-y-6">
        ...
        {isAnalyzingVideo && (
          <div className="flex flex-col items-center gap-3 py-6 text-center">
            ...
            <p className="text-sm text-muted-foreground">Analisando vídeo, isso pode levar até 1 minuto...</p>
          </div>
        )}
        ...
        {videoResult && !videoResult.error && (
          <div className={`rounded-2xl border p-5 ${
            ...
            <p className="font-bold text-foreground capitalize mb-1">{videoResult.risk_level}</p>
            <p className="text-sm text-muted-foreground mb-3">{videoResult.summary}</p>
            {videoResult.red_flags?.length > 0 && (
              <ul className="text-sm text-muted-foreground space-y-1 list-disc list-inside">
                {videoResult.red_flags.map((flag, i) => <li key={i}>{flag}</li>)}
              </ul>
            )}
        ...
        {videoResult?.error && (
          <p className="text-sm text-red-400 text-center">{videoResult.error}</p>
        )}
      </div>
