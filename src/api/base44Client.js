...
export const base44 = {
  ...
  integrations: {
    Core: {
      ...
      async InvokeVideoLLM({ prompt, video_url, response_json_schema }) {
        const { data: { session } } = await supabase.auth.getSession();
        const response = await fetch('/api/analyzeVideo', {
          ...
          body: JSON.stringify({ prompt, video_url, response_json_schema }),
        });
        if (!response.ok) {
          const err = await response.json();
          throw new Error(err.error || 'Erro na análise de vídeo');
        }
        return await response.json();
      },
