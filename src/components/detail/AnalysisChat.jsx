...
${analysis.analysis_type === "deepfake" ? `- Probabilidade de deepfake: ${analysis.deepfake_probability}%\n- Artefatos: ${(analysis.deepfake_artifacts || []).join(", ")}` : ""}
    ...
    const response = await base44.integrations.Core.InvokeLLM({
      prompt: `Você é o ScanShield, assistente de segurança digital. Responda de forma clara, direta e em português brasileiro. Baseie-se no contexto da análise fornecida.
...
Responda de forma útil e objetiva à última mensagem do usuário.`,
    });

    const reply = typeof response === "string"
      ? response
      : response?.raw || response?.text || response?.content || JSON.stringify(response);
