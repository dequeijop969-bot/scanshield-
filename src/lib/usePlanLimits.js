...
const FREE_VIDEO_KEY = "scanshield_free_video_used";

export function hasFreeVideoTrial() {
  try { return localStorage.getItem(FREE_VIDEO_KEY) !== "true"; } catch { return false; }
}

export function consumeFreeVideoTrial() {
  try { localStorage.setItem(FREE_VIDEO_KEY, "true"); } catch {}
}
...
export function getPlanLimits(user) {
  ...
  if (typeof window !== "undefined" && localStorage.getItem(STUDENT_KEY) === "true") {
    return { analyses: Infinity, questions: Infinity, deepfake: true, video: true, videoAnalyses: Infinity, linkAnalyses: Infinity, plan: "estudante" };
  }
  ...
  if (user.is_premium) {
    ...
    if (plan.includes("pro")) return { analyses: 20, questions: 10, deepfake: true, video: true, videoAnalyses: 10, linkAnalyses: 10, plan: "pro" };
    if (plan.includes("intermediario") || plan.includes("intermediário")) return { analyses: 10, questions: 8, deepfake: true, video: true, videoAnalyses: 5, linkAnalyses: 5, plan: "intermediario" };
    return { analyses: 5, questions: 5, deepfake: false, video: false, videoAnalyses: 0, linkAnalyses: 3, plan: "iniciante" };
  }

  return { analyses: 3, questions: 3, deepfake: false, video: false, videoAnalyses: 0, linkAnalyses: 3, plan: "gratuito" };
}
