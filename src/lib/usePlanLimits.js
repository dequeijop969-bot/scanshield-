...
export function getPlanLimits(user) {
  ...
  if (typeof window !== "undefined" && localStorage.getItem(STUDENT_KEY) === "true") {
    return { analyses: Infinity, questions: Infinity, deepfake: true, plan: "estudante" };
  }

  if (!user) return { analyses: 1, questions: 1, deepfake: false, linkAnalyses: 1, plan: "gratuito" };

  if (user.is_premium) {
    const plan = (user.plan_name || "").toLowerCase();
    if (plan.includes("pro")) return { analyses: 20, questions: 10, deepfake: true, linkAnalyses: 10, plan: "pro" };
    if (plan.includes("intermediario") || plan.includes("intermediário")) return { analyses: 10, questions: 8, deepfake: true, linkAnalyses: 5, plan: "intermediario" };
    return { analyses: 5, questions: 5, deepfake: false, linkAnalyses: 3, plan: "iniciante" };
  }

  return { analyses: 1, questions: 1, deepfake: false, linkAnalyses: 1, plan: "gratuito" };
}
